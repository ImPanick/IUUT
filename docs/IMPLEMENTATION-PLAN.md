...
- **Stash builder** — DONE (add w/ stack cap 100, remove, durability view + repair, **visual tile
  grid** colour-coded by category; **picker catalog = WORKSHOP items only** — `items.json` is the 377
  `Item.Meta.*`-tagged `D_ItemsStatic` rows (the orbital stash rejects raw resources / building pieces,
  so they're excluded); every blueprint-crafted Meta item is included). **Friendly names** — DONE: `items.json` enriched with 90 verified
