...
| --- | --- | --- | --- | --- |
| `talents.json` | `D_Talents` | `rowName` → `displayName` (+ optional `live`, `maxRank`) | `CatalogLoader` | **2221 rows (Week 242 / 2026-07-24): 2211 live + 10 `live:false`; 901 rows carry `maxRank` = the live `Rewards` count (one entry per rank — the row's true max; rows without it are unlock-style and clamp to the permissive fallback 4).** A **superset** — when the weekly re-mine drops a row (staged/dev-gated content cooked out, or a removed row) it is kept with `"live": false` instead of deleted, so saves referencing it still get a friendly name and the "show unreleased" toggle can reveal it. `LiveRowNames`/`IsLive` expose the live subset (Lazy Max + the blueprint checklist use it). All `Workshop_*`/`Prospect_*` unlocks; display names null → humanized. |
| `items.json` | `D_ItemsStatic` | `rowName` → `displayName` | `CatalogLoader` | **Workshop/orbital items only — `Item.Meta`/`Item.Meta.*`-tagged rows (381, live Week 242; dev/test excluded).** The Stash "Add item" picker. The orbital stash REJECTS non-Meta items (raw resources, building pieces). 90 curated names preserved; the rest humanize. |
