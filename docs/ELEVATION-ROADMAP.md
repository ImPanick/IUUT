...
| **Click-and-deploy relocation (map)** | transformative / medium | Owner's call, 2026-08-07: the map is where relocation belongs — pick a build, click a destination, it moves. v2.12.0 already supplies the write; the map supplies the X/Y. **The gap is Z**, and the save answers it: every *world* actor (resource deposits, voxels, cave entrances) sits on or in the terrain, and v2.11.0 decodes them all, so the blob carries a scattered height field for free. Measured over 6 real prospects — 934–2,207 placed world actors each, covering the full ~7.5 km map — inverse-distance weighting over the 8 nearest samples predicts a *known* build height to **median 0.6–1.7 m error, p90 ≤ 4.8 m**, i.e. within a foundation's thickness. **Known limits before this ships:** cave entrances and cave AI are *below* ground and bias an estimate downward (weight `ResourceDeposit` samples, or reject outliers); sample density is high near bases (median nearest sample 1.3–6.4 m) precisely because players build near resources, so open wilderness will be sparser; and IDW smooths cliffs, so a destination on steep ground needs a low-confidence warning rather than a silent snap. Surface the confidence (distance to nearest samples) in the UI — never present an estimate as a survey. |
