...
| **Homestead pack-up / base relocation** | transformative / large | **Read-only milestone SHIPPED (v2.10.0):** `ProspectHomesteadReader` classifies player-built actors (deployables/beds/building grids) and measures the move's dependencies — actor-id space (1,117 ids, max 823,457 on the sample), `FoundationActorIcarusUID` anchors (18 of 26 pieces) + tame whitelists, and 28 bound terrain tiles; surfaced in `iuut prospect-report`. **Transform decode SHIPPED (v2.11.0):** Icarus writes `ActorTransform` as TAGGED sub-properties (Rotation/Translation/Scale3D), decoded by `ProspectTransformReader` — deliberately targeted rather than widening the reader's native-struct set, which is shared with the writer. Rebasing is therefore an IN-PLACE 12-byte write (no length fixup). Measured: bases are NOT single clusters (460 structures over 7 km on one save), so pack-up needs spatial/connected selection. **NEXT (write phase):** id remapping on inject + connected-set selection so foundation links never dangle — gated on lossless round-trip fixtures before any announcement. Paywalled at ~$4/28-days elsewhere. |
