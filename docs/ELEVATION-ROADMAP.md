...
| Mount rescue suite | high / medium | Proven demand ("Mount Reviver Needed!"). Roster restore/clone **shipped v2.3.0**; in-prospect rename **shipped v2.7.0** (first length-changing blob write — the fixup path homestead pack-up needs, now proven). Remaining: level/XP (needs recorder field decode). |
...
| Item | Impact / Effort | Notes |
| --- | --- | --- |
| Mission/quest reset inside prospect saves | high / large | **SHIPPED (v2.4.0 read, v2.5.0 write):** `ProspectQuestReader` + `ProspectQuestEditor` (in-place, size-preserving zeroing; surgical-write gates count the exact differing bytes; real-blob gate passed on a scratch copy). `iuut prospect-report` + `iuut quest-reset --apply`. App panel: **shipped v2.6.0** (Prospect Quests, WORLD group). The category leader's flagship paid feature, offline and free. |
| **Homestead pack-up / base relocation** | transformative / large | **Read-only milestone SHIPPED (v2.10.0):** `ProspectHomesteadReader` classifies player-built actors (deployables/beds/building grids) and measures the move's dependencies — actor-id space (1,117 ids, max 823,457 on the sample), `FoundationActorIcarusUID` anchors (18 of 26 pieces) + tame whitelists, and 28 bound terrain tiles; surfaced in `iuut prospect-report`. **Transform decode SHIPPED (v2.11.0):** Icarus writes `ActorTransform` as TAGGED sub-properties (Rotation/Translation/Scale3D), decoded by `ProspectTransformReader` — deliberately targeted rather than widening the reader's native-struct set, which is shared with the writer. Rebasing is therefore an IN-PLACE 12-byte write (no length fixup). Measured: bases are NOT single clusters (460 structures over 7 km on one save), so pack-up needs spatial/connected selection. **In-prospect relocation SHIPPED (v2.12.0):** `HomesteadSurvey.Clusters()` groups pieces into builds by single-link clustering (60 m default) and `ProspectHomesteadEditor` offsets a whole build in place; `iuut homestead-move` lists builds and relocates one, preview-first. Safe because a same-world move needs none of the three hard parts — actor ids are untouched (no collisions, no dangling `FoundationActorIcarusUID`), whitelists are untouched, and player-built structures carry no FLOD terrain binding (`TileName=None`, indices `-1`, verified on every real prospect). Real-blob gate: 23-piece base moved 250 m; blob byte-length identical, 201 item slots / 11 quest steps / other builds all unchanged. **NEXT (cross-prospect):** id remapping on inject so links survive a different world — still gated on lossless round-trip fixtures before any announcement. Paywalled at ~$4/28-days elsewhere. |
| Offline map + deployable viewer | medium / large | **Unblocked by v2.11.0** — world positions now decode (`ProspectTransformReader`), so plotting is a UI task rather than a research one. Plot containers/deployables from the local blob over terrain maps; click a crate → see contents → jump to Return-to-Stash. The privacy-preserving alternative to upload-based cartographers. |
| **Click-and-deploy relocation (map)** | transformative / medium | Owner's call, 2026-08-07: the map is where relocation belongs — pick a build, click a destination, it moves. v2.12.0 supplies the write, the map supplies X/Y, and **the Z gap is now closed: `TerrainHeightField` SHIPPED (v2.13.0)**, surfaced in `homestead-move` with a confidence grade and a `--snap` offset. Remaining work is UI only. **Ground rule for the panel:** show the confidence grade and its plain-English reason next to any proposed drop, and refuse to present a Low-confidence estimate as a placement — every catastrophic miss in calibration (up to 191 m) sat in that bucket. |
| ~~Extract the game's shipped heightmaps~~ | **CLOSED — do not re-investigate** | The install *does* ship real landscape heightmaps: `Icarus/Content/Maps/Terrain_0NN/heightmap/heightmap_xN_yM.{umap,uexp,ubulk}`, UE4 Landscape streaming proxies tiled per terrain (Terrain_019_DLC 22 tiles, Terrain_022 23, Terrain_021_DLC2 16, plus outposts), alongside `T_LandscapeStreamingProxy_*_D` diffuse textures that would make an excellent map backdrop. **They are unreachable.** The pakchunks are UE4 pak v11 with an *unencrypted* index (so enumeration works — that is how the above was measured), but the entries themselves are Oodle-compressed (3,361 of 3,895 heightmap entries; method #1 = Oodle). No `oo2core*.dll` ships with the game — Oodle is statically linked into the executable — so there is no user-owned copy to P/Invoke, and bundling Oodle fails both the dependency gate (SCOPE_GUARDRAILS §2.6) and its licence. No pure-managed Kraken decoder exists. Conclusion: the world-actor height field is not a stopgap, it is the answer. |

| **Live inventory editor, laid out like the game** | high / large | Owner's ask, 2026-08-07: edit a character's in-prospect inventory in a panel that mirrors the game screen (bag grid, equipment doll, envirosuit, Auxiliary modules), including suit-granted slots. **DECODED (read-only research, v2.14.0 groundwork):** a character carries SIX inventories under `SavedInventories`, with stable ids identified from real saves by their contents — **2** = hotbar/toolbelt (axe, pickaxe, bow, `Player_Fist`), **3** = the main INVENTORY grid, **4** = the consumable column (Oxygen_Tank, Thermos, Oxite), **5** = EQUIPMENT, a fixed 9 in fixed order `[0]Head [1]Chest [2]Arms [3]Legs [4]Feet [5]Envirosuit [6]Skin [7]Cap [8]Backpack/Quiver`, **11** = AUXILIARY modules (`Meta_Module_Temperature/Caving/Extremes` — the panel in the owner's screenshot), **12** = lantern. Per-slot payload: `Location` (grid index), `ItemStaticData` (row), `ItemGuid`, `ItemOwnerLookupId`, `DynamicData` (Index/Value pairs — durability), `AdditionalStats`, `Alterations`. **THE KEY CONSTRAINT:** slots are stored SPARSELY — only occupied ones, addressed by `Location` — proven by a character whose consumable inventory holds exactly two entries at Location 0 and 2. **The save therefore does not record capacity**, so drawing the right number of empty cells needs the game's own data: `InventoryInfo.StartingSlots` in data.pak (mineable today by `DataPakMiner`; e.g. `Storage_Homestead_Cabinet`=45, `Pouch_Generic`=6). **CAPACITY MODEL RESOLVED (v2.15.0):** suits grant slots as STATS, not inventory declarations. `ItemStaticData[suit].Armour` → `ArmourData.ArmourStats`, e.g. `Undersuit_Larkwell_Alpha` = `{BaseUpgradeSlots_+: 4, BaseBackpackSlots_+: 6, BaseOxygenSlots_+: 1}` — exactly the +6 inventory / +4 module the game shows. Stat meanings are in `IcarusStatDescription` (`BaseBackpackSlots_+` = "+{0} Suit Inventory Slots", `BaseUpgradeSlots_+` = "+{0} Suit Module Slots", plus Vision/Generic/Saddle/MountCargo variants). Base sizes: `InventoryInfo.StartingSlots` (Backpack 24, Equipment 10, `Player_Grave` 70). **Talents add more on top** — a real character holds items out to Location 35 against 24+6=30 from base+suit — so the grid MUST draw `max(occupied+1, base+granted)`; anything tighter hides items. **Reader SHIPPED v2.15.0** (`ProspectInventoryReader`, `--inventory`); remaining work is the WPF panel plus a capacity catalog. |
